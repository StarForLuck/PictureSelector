...
public class PicturePreviewAdapter extends RecyclerView.Adapter<BasePreviewHolder> {
     ...
     * @param position
     */
    public void startAutoPlay(int position) {
        BasePreviewHolder currentHolder = getCurrentHolder(position);
        if (currentHolder instanceof PreviewVideoHolder) {
            PreviewVideoHolder videoHolder = (PreviewVideoHolder) currentHolder;
            if (!videoHolder.mediaPlayer.isPlaying()) {
                videoHolder.ivPlayButton.performClick();
            }
        } else if (currentHolder instanceof PreviewAudioHolder){
            PreviewAudioHolder audioHolder = (PreviewAudioHolder) currentHolder;
            if (!audioHolder.mediaPlayer.isPlaying()) {
                ImageView viewPlay = audioHolder.controller.getViewPlay();
                if (viewPlay != null) {
    ...
    public boolean isPlaying(int position) {
        BasePreviewHolder currentHolder = getCurrentHolder(position);
        if (currentHolder instanceof PreviewVideoHolder) {
            return ((PreviewVideoHolder) currentHolder).mediaPlayer.isPlaying();
        } else if (currentHolder instanceof PreviewAudioHolder) {
            return ((PreviewAudioHolder) currentHolder).mediaPlayer.isPlaying();
        }
        return false;
    }
