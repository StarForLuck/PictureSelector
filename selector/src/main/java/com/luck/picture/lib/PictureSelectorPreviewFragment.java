...
public class PictureSelectorPreviewFragment extends PictureCommonFragment {
    ...
    protected boolean isPlayPageSelected = false;
    ...
    protected void onMojitoBeginBackMinAnim() {
        ...
        if (currentHolder instanceof PreviewVideoHolder) {
            ...
            if (videoHolder.controller != null) {
                if (((View) videoHolder.controller).getAlpha() != 0F) {
                    ((View) videoHolder.controller).animate().alpha(0F).setDuration(125).start();
                }
            }
        }
    }
                        ...
                        viewPager.post(new Runnable() {
                            ...
                            public void run() {
                                if (selectorConfig.isPreviewZoomEffect) {
                                    ...
                                    if (currentHolder instanceof PreviewVideoHolder) {
                                        PreviewVideoHolder videoHolder = (PreviewVideoHolder) currentHolder;
                                        if (!videoHolder.mediaPlayer.isPlaying()) {
                                            videoHolder.ivPlayButton.setVisibility(View.VISIBLE);
                                        }
                                    }
                                }
    ...
    private void startAutoVideoPlay(int position) {
        Looper.myQueue().addIdleHandler(new MessageQueue.IdleHandler() {
            ...
            public boolean queueIdle() {
                viewPageAdapter.startAutoPlay(position);
                return false;
            }
        });
    }
    ...
    private void resumePausePlay() {
        ...
        BasePreviewHolder currentHolder = viewPageAdapter.getCurrentHolder(viewPager.getCurrentItem());
        if (currentHolder instanceof PreviewVideoHolder) {
            if (((PreviewVideoHolder) currentHolder).mediaPlayer.isPlaying()) {
                ((PreviewVideoHolder) currentHolder).mediaPlayer.pause();
            } else {
                ((PreviewVideoHolder) currentHolder).mediaPlayer.resume();
            }
        }
        if (currentHolder instanceof PreviewAudioHolder) {
            if (((PreviewAudioHolder) currentHolder).mediaPlayer.isPlaying()) {
                ((PreviewAudioHolder) currentHolder).mediaPlayer.pause();
            } else {
                ((PreviewAudioHolder) currentHolder).mediaPlayer.resume();
            }
        }
    }
